...
import { YouSaidWeImproved } from './pages/feedback/YouSaidWeImproved';
import { ComplaintForm } from './pages/feedback/ComplaintForm';
import { ComplaintTracker } from './pages/feedback/ComplaintTracker';
import { FarmerPlatformFeedback } from './pages/feedback/FarmerPlatformFeedback';
import { BuyerFeedbackForm } from './pages/feedback/BuyerFeedbackForm';
import { FarmerBuyerFeedbackForm } from './pages/feedback/FarmerBuyerFeedbackForm';
...
const PUBLIC_TABS = ['home', 'landing', 'login', 'register', 'traceability', 'tracking', 'support', 'you-said-we-improved', 'improvements'];
...
const PageContent: React.FC = () => {
  const { activeTab, setActiveTab, currentRole, isAuthenticated, attemptedFeature } = useApp();
  ...
  switch (activeTab) {
    ...
    // ── Feedback Intelligence & Complaints System ────────────────
    case 'feedback-intelligence':
      if (currentRole !== 'ADMIN') {
        return <AccessDenied attemptedFeature="Platform Feedback Intelligence Center" />;
      }
      return <FeedbackIntelligencePage />;

    case 'delivery-feedback':
      if (currentRole !== 'LOGISTICS' && currentRole !== 'ADMIN') {
        return <AccessDenied attemptedFeature="Logistics & Fleet Delivery Feedback" />;
      }
      return <FeedbackIntelligencePage />;

    case 'hub-feedback':
      if (currentRole !== 'FPO_AGGREGATOR' && currentRole !== 'ADMIN') {
        return <AccessDenied attemptedFeature="Micro-Hub & FPO Feedback Console" />;
      }
      return <FeedbackIntelligencePage />;

    case 'buyer-feedback':
    case 'feedback-rate-order':
      if (currentRole !== 'RETAIL_BUYER' && currentRole !== 'BULK_BUYER' && currentRole !== 'ADMIN') {
        return <AccessDenied attemptedFeature="Buyer Produce Feedback Form" />;
      }
      return (
        <div className="max-w-xl mx-auto px-4 py-6">
          <BuyerFeedbackForm onClose={() => setActiveTab('dashboard')} />
        </div>
      );

    case 'rate-buyer':
      if (currentRole !== 'FARMER' && currentRole !== 'ADMIN') {
        return <AccessDenied attemptedFeature="Farmer-to-Buyer Evaluation Form" />;
      }
      return (
        <div className="max-w-xl mx-auto px-4 py-6">
          <FarmerBuyerFeedbackForm onClose={() => setActiveTab('dashboard')} />
        </div>
      );

    case 'farmer-platform-feedback':
      if (currentRole !== 'FARMER' && currentRole !== 'ADMIN') {
        return <AccessDenied attemptedFeature="Farmer Platform Feedback" />;
      }
      return (
        <div className="max-w-2xl mx-auto px-4 py-6">
          <FarmerPlatformFeedback onClose={() => setActiveTab('dashboard')} />
        </div>
      );
    ...
    case 'report-problem':
      return (
        <div className="max-w-xl mx-auto px-4 py-6">
          <ComplaintForm onClose={() => setActiveTab('dashboard')} />
        </div>
      );
    ...
    case 'you-said-we-improved':
      return <YouSaidWeImproved />;
