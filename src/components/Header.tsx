  ...
  return (
    <header className="sticky top-0 z-30 h-16 bg-white border-b border-slate-200/80 flex items-center px-4 gap-3 shadow-2xs">
      ...
      <div className="hidden sm:block min-w-0">
        <h1 className="text-sm font-semibold text-slate-900 truncate tracking-tight">{pageTitle}</h1>
      </div>
      ...
      <div className="flex items-center gap-2 ml-auto">
        ...
        {!isOnline ? (
          <button
            ...
            className="flex items-center gap-1.5 bg-amber-50 hover:bg-amber-100 text-amber-900 border border-amber-200 px-3 py-1 rounded-xl text-[11px] font-semibold shadow-2xs transition cursor-pointer"
          ...
          <div className="flex items-center gap-1.5 bg-blue-50 text-blue-900 border border-blue-200 px-3 py-1 rounded-xl text-[11px] font-semibold animate-pulse">
            <RefreshCw className="w-3.5 h-3.5 text-blue-600 animate-spin shrink-0" />
            <span className="hidden sm:inline">Syncing changes...</span>
            <span className="sm:hidden">Syncing</span>
          </div>
        ) : syncStatus === 'synced' ? (
          <div className="flex items-center gap-1.5 bg-emerald-50 text-emerald-900 border border-emerald-200 px-3 py-1 rounded-xl text-[11px] font-semibold">
        ...
        {isInstallable && (
          <button
            ...
            className="hidden md:flex items-center gap-1.5 bg-slate-900 hover:bg-slate-800 text-white px-3 py-1.5 rounded-xl text-xs font-semibold transition shadow-xs cursor-pointer"
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-1.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-800 px-3 py-1.5 rounded-xl text-xs font-semibold transition cursor-pointer"
            ...
            <div className="absolute right-0 top-full mt-1 w-52 bg-white rounded-2xl shadow-lg border border-slate-200/90 z-50 overflow-hidden py-1">
              ...
              <button
                ...
                className={`w-full text-left px-4 py-2.5 text-xs font-medium transition cursor-pointer ${
                  currentRole === 'FARMER' ? 'bg-emerald-50 text-emerald-900 font-semibold' : 'text-slate-700 hover:bg-slate-50'
                }`}
              >
                🌾 Farmer Portal
              </button>
              <button
                ...
                className={`w-full text-left px-4 py-2.5 text-xs font-medium transition cursor-pointer ${
                  currentRole === 'RETAIL_BUYER' ? 'bg-emerald-50 text-emerald-900 font-semibold' : 'text-slate-700 hover:bg-slate-50'
                }`}
              >
                🛒 Buyer Portal
              </button>
              <button
                ...
                className={`w-full text-left px-4 py-2.5 text-xs font-medium transition cursor-pointer ${
                  currentRole === 'ADMIN' ? 'bg-emerald-50 text-emerald-900 font-semibold' : 'text-slate-700 hover:bg-slate-50'
                }`}
              >
                🛡️ Operations / Admin
              </button>
        ...
        <div className="relative">
          ...
          >
            {/* Avatar */}
            <div className="w-7 h-7 rounded-lg bg-emerald-700 flex items-center justify-center text-xs font-bold text-white shadow-2xs shrink-0">
              {currentUser.avatar || '👤'}
            </div>
            <div className="hidden sm:block text-left">
              <p className="text-xs font-semibold text-slate-800 leading-tight">{(currentUser.name || 'User').split(' ')[0]}</p>
            </div>
          ...
          {isUserMenuOpen && (
            <div className="absolute right-0 top-full mt-1 w-52 bg-white rounded-2xl shadow-lg border border-slate-200/90 z-50 overflow-hidden py-1">
              ...
              <div className="px-4 py-3 border-b border-slate-100">
                ...
                <span className="mt-1.5 inline-block text-[9px] font-semibold px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-800 border border-emerald-200">
              ...
              {isInstallable && (
                <button
                  ...
                  className="w-full flex items-center gap-2.5 px-4 py-2.5 text-xs text-emerald-800 font-semibold bg-emerald-50 hover:bg-emerald-100 transition text-left cursor-pointer"
                >
