...
import { QrCode, Search, CheckCircle2, Clock, Package, Truck, MapPin, ExternalLink } from 'lucide-react';
// passport data available via openPassportModal
...
export const TraceabilityPage: React.FC = () => {
  const { openPassportModal } = useApp();
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedBatchId, setSelectedBatchId] = useState<string | null>('AGP-TOM-2026-001');

  const batches = [
    { batchId: 'AGP-TOM-2026-001', crop: 'Tomato', farmerOrFpo: 'GreenHarvest FPO', harvestDate: '06 Sep 2026', quantityKg: 3000, status: 'In Transit', destination: 'ABC Retail — Chennai Terminal' },
    { batchId: 'AGP-TOM-2026-002', crop: 'Tomato', farmerOrFpo: 'Rajesh Kumar', harvestDate: '05 Sep 2026', quantityKg: 800, status: 'Quality Checked', destination: 'Grand Hospitality Group' },
    { batchId: 'AGP-CAR-2026-003', crop: 'Carrot', farmerOrFpo: 'Murugan FPO', harvestDate: '04 Sep 2026', quantityKg: 1200, status: 'Delivered', destination: 'Metro Cash & Carry, T. Nagar' },
    { batchId: 'AGP-ONI-2026-004', crop: 'Onion', farmerOrFpo: 'Kaveri Cluster', harvestDate: '03 Sep 2026', quantityKg: 5000, status: 'Packed', destination: 'Chennai APMC Yard' },
  ];
  ...
  const selectedBatch = selectedBatchId ? batches.find((b) => b.batchId === selectedBatchId) : null;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-4xl font-anton text-forest tracking-wide">Produce Traceability</h1>
          <p className="text-sm font-medium text-forest/70 mt-1">Track every batch from harvest to delivery</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-4">
          <h3 className="text-[10px] font-bold text-forest uppercase tracking-widest">Active Batches ({filtered.length})</h3>
          {filtered.map((b) => (
            <button
              key={b.batchId}
              onClick={() => setSelectedBatchId(b.batchId === selectedBatchId ? null : b.batchId)}
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <p className="font-anton text-forest text-xl mt-3 tracking-wide">{b.crop} — {b.quantityKg.toLocaleString()} kg</p>
              ...
              <div className="mt-4 pt-4 border-t border-olive/20 flex items-center gap-4 text-[10px] font-bold uppercase tracking-widest text-forest/60">
                ...
                <span className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" />{b.destination.split('—')[0].trim()}</span>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {[
                  ...
                  <div key={item.label} className="p-4 bg-olive/10 rounded-[1.5rem] border border-olive/30 shadow-sm">
                    ...
                    <p className="font-anton text-xl text-forest mt-1 tracking-wide">{item.value}</p>
                  </div>
                ))}
              </div>
